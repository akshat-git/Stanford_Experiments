...
# === Top Eigenvalue Score (no weighting) ===
def top_eigen_score(batch_indices, model):
...
def make_objective(model, batch_size):
    ...
    def objective(trial):
        ...
        return -eigvals[-1].item()  # maximize top eigenvalue

    return objective

# === Training Loop ===
def train(model, use_optuna=True, label="TopEig", epochs=40, reoptimize_every=4, step_size=0.001, batch_size=2):
    ...
    for epoch in range(epochs):
        ...
        if use_optuna and epoch % reoptimize_every == 0:
            ...
            _, top_vec = top_eigen_score(best_batch, model)
...
batch_size = 2
eig_model = TinyTransformerLM(vocab_size).cpu()
rand_model = TinyTransformerLM(vocab_size).cpu()

eig_losses = train(eig_model, use_optuna=True, label="TopEig", batch_size=batch_size)
rand_losses = train(rand_model, use_optuna=False, label="Random", batch_size=batch_size)

# === Plot Comparison ===
plt.plot(eig_losses, label="Top Eigenvalue Selection", marker='o')
plt.plot(rand_losses, label="Random Selection", marker='x')
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Top Eigenvalue Batch Selection vs Random Batch Training")
plt.legend()
plt.grid(True)
plt.show()
