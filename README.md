...
- `test/demo.py` is the clean multi-pass demo: reads `config.py` (tunable via `--passes`/`--group`; CLI equations are added to the set), writes per-pass thoughts to `run_log.txt`, prints loss/reward/accuracy + a final per-task mode-of-answers summary, and saves `performance_graphs.png` (needs matplotlib).
...
- `real_model/demo.py` is the clean multi-pass demo: reads `config.py` (tunable via `--passes`/`--group`; CLI equations are added to the set), writes per-pass thoughts to `run_log.txt`, prints loss/reward/accuracy + a final per-task mode-of-answers summary, and saves `performance_graphs.png` (needs matplotlib).
